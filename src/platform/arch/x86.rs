...
//     let mut vendor_buf =
//         // SAFETY:
//         // transmute is just changing a [u32; 3] to [u8; 12]. The size
// remains the         // same, no invalid types can be created, so it should
// always be safe.         unsafe { transmute::<_, [u8; 12]>([res.ebx, res.ecx,
// res.edx]) }             .to_vec();
//     vendor_buf.push(b'\0');
...
//     // SAFETY:
//     // transmute is just changing a [u32; 12] to [u8; 48]. The size remains
// the     // same, no invalid types can be created, so it should always be
// safe.     let mut name_buf = unsafe {
//         transmute::<_, [u8; 48]>([
