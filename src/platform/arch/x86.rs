use std::{mem::transmute, ffi::CStr};
...
pub fn detect_cpu_vendor_and_name() -> (Option<String>, Option<String>) {
    ...
    let mut vendor_buf = unsafe { transmute::<_, [u8; 12]>([ res.ebx, res.ecx, res.edx ]) }.to_vec();
    vendor_buf.push(b'\0');
    ...
    let mut name_buf = unsafe { transmute::<_, [u8; 48]>([ 
        res_1.eax, res_1.ebx, res_1.ecx, res_1.edx,
        res_2.eax, res_2.ebx, res_2.ecx, res_2.edx,
        res_3.eax, res_3.ebx, res_3.ecx, res_3.edx,
    ]) }.to_vec();
    name_buf.push(b'\0');
