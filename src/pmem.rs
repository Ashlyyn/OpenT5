...
use windows::Win32::System::Memory::{VirtualAlloc, PAGE_READWRITE, MEM_COMMIT};

#[cfg(any(target_os="unix", target_os="linux"))]
use nix::sys::mman::{mmap, ProtFlags, MapFlags};

#[cfg(all(not(target_os="windows"), not(target_os="unix"), not(target_os="linux")))]
use libc::malloc;
...
impl<'a, 'b, 'c, 'd> PhysicalMemory<'a, 'b, 'c, 'd> {
    fn new(n: &'a str, b: Option<&'b mut [u8]>, s: usize) -> Self {
        PhysicalMemory {
            name: n,
            buf:  b,
            prim: [PhysicalMemoryPrim::<'c, 'd>::new("", 0, 0, MemTrack::DEBUG), 
...
fn alloc<'a>(size: usize) -> Option<&'a mut [u8]> {
    let p = unsafe { VirtualAlloc(core::ptr::null(), size, 
                                    MEM_COMMIT, PAGE_READWRITE) as *mut u8 };
    match p.is_null() {
        true =>  None,
...
#[cfg(any(target_os="unix", target_os = "linux"))]
fn alloc<'a>(size: usize) -> Option<&'a mut [u8]> {
    let p = unsafe { mmap(core::ptr::null_mut(), size, 
                          ProtFlags::PROT_READ  | ProtFlags::PROT_WRITE, 
                          MapFlags::MAP_PRIVATE | MapFlags::MAP_ANON, 0, 0).unwrap() as *mut u8 };
    match p.is_null() {
        true => None,
        false => unsafe { Some(core::slice::from_raw_parts_mut(p, size)) }
    }
...
lazy_static! {
    ...
    static ref G_MEM: RwLock<PhysicalMemory<
